...
| ID | Task | State |
|----|------|-------|
| DECIDE-1 | Does Scheme [54] belong in Exp. 4? | **CLOSED 2026-09-06** — §V now names Schemes [30], [35], [54] for Exp. 4, which is exactly the measured roster, and states why [54] is absent from panel (b). Audited: all five experiments cite exactly what they measure. |
| DECIDE-2 | What statistic do the baselines get reported with? | **DECIDED 2026-09-06: the mean.** README §7's `mean ± 95% CI` stands for every scheme; medians are not substituted for two baselines only. **Half-applied** — the disclosure the decision depends on is NOT yet in §V. It must say that Scheme35 Exp. 3 and Scheme30 Exp. 2 are heavy-tailed (mean/median 2.19 and 16.9), that this is output-sensitivity and not a defect, and that Scheme35's d-scaling is visible in the medians (36.6 / 21.3 / 18.5 ms at d=2/6/10) but flat under the mean. Reporting the mean without saying so publishes a number that hides a baseline's own finding. See `remainfix.txt:125` |
...
| BUILD-2b | Batch Exp. 4's chain lookup: one ledger query for all records, not one per record | **LANDED `e252cc9`, measured `d3452c0`: 2.19x, and the O(r) chain term survives.** This row said `open` until 2026-09-06 while `Exp4Verification.measure` had been calling `vledger_mod.batched_chain_checker` since `e252cc9`. The batching target was met; reaching 2nd was NOT — that still needs BUILD-2's aggregate proof alongside it. Original note: Floor becomes ~6.15 ms host. Also the only version that survives BUILD-1: 167 real Fabric RPCs would dominate everything |
