...
| `feng_bl_abse/` | Ref[57] | Feng *et al.*, *Electronics* (MDPI) 2026 | Blockchain-enabled lattice ABSE with instant revocation — **not yet implemented**, added 2026-08-28 to give Exp. 6 a baseline |
...
- **Ref[57]** — λ=128, RLWE with `n=1024`, `q=12,289` (the smallest prime satisfying the NTT constraint `q ≡ 1 mod 2n`), `σ=3.2`, commitment-match threshold `B=40`, `(3,5)` threshold key sharing, Hyperledger Fabric/PBFT at a 2 s block time, IPFS storage, AES-256-GCM. **Every cryptographic parameter is published numerically** — unusually complete, so unlike Ref[41] and Ref[54] there are no undetermined values to fix. The open items are scoping decisions (Exp. 3 inclusion, Fabric dependency for Exp. 6, native-mode multi-keyword) recorded in `Schemes/feng_bl_abse/SCHEME.md`.
...
| 6 | Authorization Sync | updates `δ` | 10² → 10⁵ | latency (ms) | IAS message size (KB), FSNs touched | Ours, Ref[57] |
...
Per-scheme experiment coverage: ours 1–8 · Guo 1,2,3,4,5 · Thingom 1,2,3 · Perera & Fugkeaw (Ref[54]) 1,2,3 · Feng *et al.* (Ref[57]) 1,2,6 — the last two **not yet implemented**, see their `SCHEME.md` files. Ref[54] does not cover Exp. 5/6 (no incremental-update primitive; self-disclosed no fine-grained revocation, only coarse epoch-based key evolution) — narrower than Zhuang's old slot, deliberately, not copied over. Ref[57] covers Exp. 6, which no other baseline could; its Exp. 3 participation is **deferred pending a decision** (its system model has no domain concept) and Exp. 5 is excluded for the same no-update-primitive reason as Ref[54]. XB-Muse (Ref[36]) was **dropped on 2026-08-23** — see §14 item 8. Zhuang (Ref[52]) was **dropped and replaced by Ref[54] on 2026-08-27** — see §16.
