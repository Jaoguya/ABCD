...
> Manuscript: [Overleaf/MA-LB-PQ-VDSE.tex](Overleaf/MA-LB-PQ-VDSE.tex)
...
| VII | Dependency-Aware Dynamic Evolution & Incremental Auth. Sync (DIAS) | Exp. 5, 6 |
| VIII | Verifiable Retrieval | Exp. 4 |

The three novelty claims — PDSI, AASS, DIAS — are measured by Exp. 2, Exp. 7–8, and Exp. 6 respectively.
...
|---|-----------|----------|-------|---------------|-----------|---------|
| 1 | Policy-State-Aware Token Generation | keywords `q` | 1 → 20 | latency (ms) | trapdoor size (B) | All 5 |
...
| 4 | Fine-Grained Verification Effectiveness | returned records `r` · tampered records `t` | 10 → 1000 · 1 → 1000 | latency (ms) · records discarded | proof size (KB), path length, records retained | Ours, Ref[30], Ref[35], Ref[54] |
| 5 | Dynamic Index Update | (keyword, doc) pairs `k` | 10² → 10⁵ | latency (ms) | Merkle nodes recomputed, entries rewritten | Ours, Ref[35] |
| 6 | DIAS Synchronization Ablation | updates `δ` | 10² → 10⁵ | latency (ms) | DIAS message size (KB), FSNs touched | Ours |
...
One variant per half of the DIAS claim, so each half has something to be measured
against:
...
- **Exp. 4** — two halves, one figure. **(a) Cost:** client-side verification only — Merkle proof, `Commit_i*` recomputation, chain consistency. IPFS fetch and decryption excluded. Reported per returned ciphertext as `T_avg = T_verify / r`, which is what §V states. **(b) What it buys:** `t` of the `r` returned results are tampered and the metric is how many the client must discard. Discarding exactly `t` is localizing exactly `t` and retaining the rest, so one curve carries both.
  - **`r` and `t` count RECORDS, never index entries.** One bundle per returned ciphertext. `build_response` emits one bundle per index *entry*, so taking all of them inflates the sweep by `|W_i|` (~32 on corpus v4) and silently compares against baselines that return records — the defect `d1cdf9c` fixed in (a) and `2026-09-05` fixed in (b). Pinned by `test_exp9_granularity.py::test_exp4_and_exp9_agree_on_what_one_returned_result_is`.
  - Panel (b) runs from `exp9_verification_granularity/`, which stays a separate sweep and a separate `results.csv`; only the *figure* is merged. Its roster is Ours, Ref[30], Ref[35] — Ref[54] verifies per record rather than by an aggregate check, so the all-or-nothing behaviour panel (b) measures does not describe it and it was not measured there. §V says so explicitly rather than leaving it absent.
...
├── Schemes/<scheme>/
│   ├── SCHEME.md
│   ├── MANUSCRIPT_DIVERGENCE.md   # ours only — where code and §V disagree
│   ├── src/                # implementation
│   │   └── psa/            # ours only — the manuscript's construction (D1–D5)
...
### The policy-state-aware construction (`--construction psa`)

The current manuscript's token is `T = H(w ‖ PID ‖ PV ‖ Dom)`; the implemented
scheme's is `T = H(w)`. Both now exist, and `--construction psa` measures the
manuscript's — see `Schemes/ma_lb_pq_vdse/MANUSCRIPT_DIVERGENCE.md` D1–D9.

```bash
python3 -m Schemes.ma_lb_pq_vdse.src.main --construction psa --experiment 1,3,5
python3 -m Schemes.ma_lb_pq_vdse.src.main --construction psa --experiment 6 --variant all
```

It writes to `psa_exp*/`, never to the Option D directories, and stamps
`construction=psa` into every `run_meta.json`. **It is not an ablation arm of the
existing experiments**: `psa_exp1` and Exp. 1 time different functions, and
`psa_exp6` sweeps a different variable, so the two must not be averaged or plotted
on one axis. Experiments 2, 4 and 7–8 have no `psa` form and the flag refuses them
rather than measuring something else.

**A `psa` number is only comparable with an Option D number from the same host.**
The banked Option D results were measured on AWS; running the two on different
machines and subtracting is the one thing this track cannot support.
...
| 11 | **The manuscript states claims the data does not support, and defines symbols it never uses.** Found 2026-09-06 while re-anchoring the code to the new revision; all verified against the `.tex`. (a) Exp. 4 calls the per-result verification cost "moderate" and lists Ref[54] among the compared schemes — the measurements say it is the **highest** of the four, and Ref[54] is *omitted* from panel (b) because its verification is already per-record; the previous revision said both. (b) Exp. 7 says concurrency runs "100 to 5000" while `global.yaml` sweeps to 10,000 and Exp. 8, which shares those runs, correctly says 10,000. (c) `\cite{refSynthea}` has no `\bibitem` and renders as `[?]`. (d) `tab:cost` uses `T_Mul`, `T_Sig`, `T_BF` and `n_cand`, none defined in `tab:cost-notation`. (e) `tab:notation` defines `VID_i` (0 uses; `PV_i` has 27) and `Score_j` (0 uses; the scheduler's symbol is `SC_j`, itself undefined). (f) The Conclusion still says "PDSI" and "IAS" where the body says DIAS. | Fix in Overleaf. (a) is the one that costs something: a reviewer who reads Fig. 4(a) will see the highest curve described as moderate, and the trade-off argument that follows does not need the softer wording. |
| 12 | **`MacOS/SETUP.md` tells you to `pip install -r requirements.txt`, and there is no `requirements.txt`.** A fresh dev host cannot follow the documented setup. The working set for the test suite is `pytest pyyaml mmh3 numpy scipy bitarray cryptography`; charm and liboqs are separate and Linux-only. | Commit a `requirements.txt`, or correct `SETUP.md` to list the packages |
| 13 | **Exp. 2, 4 and 7–8 have no policy-state-aware form.** `index/dsi.py`, `fsn/` and `verify/` are written against `types.IndexEntry` and its scalar `vid`, so the `psa` track cannot run them and the manuscript's Search and Verification `tab:cost` rows have no measurement. `--construction psa` refuses those numbers rather than measuring something else. | Fork the index/FSN/verify layer onto `psa.records.PolicyStateIndexEntry` if those rows need evidence; otherwise state in §V that they are analytical only |
| 14 | **`AA(PID_i)` is a benchmark choice, not a published one.** §V never says how many AAs govern a policy. Read literally — one AA per domain, policy ids `<domain>/polN` — the set is a **singleton**, which makes Policy-State Non-Interference vacuous rather than true. `psa/governance.py` defaults to 2-of-4. | Confirm the intended sharing model, or state the assumption in §V alongside `N_AA` and `\|A_i\|` |
...
| 2026-09-05 | **Exp. 4 redesigned as cost-plus-granularity; Exp. 9 merged into it; the manuscript's paste-back reverts undone.** §V now presents Exp. 4 as *Fine-Grained Verification Effectiveness* — panel (a) per-result latency `T_avg = T_verify/r`, panel (b) records discarded when `t` results are tampered — so the loss and what buys it are read together. The standalone Exp. 9 figure is gone; `exp9_verification_granularity/` remains a separate sweep and `results.csv`, only the figure merged. `generate_plots.py` gained cross-folder panels (`PanelSpec.folder`, its own x-axis and legend) and a `per_x` transform, and panel (b) carries its own legend because its roster is three schemes, not four. **Bug fixed: Exp. 9 swept index ENTRIES, not records** — `build_response` emits one bundle per entry and the runner `extend`ed them all, so the pinned 20,000 was ~632 records' worth of entries while guo returned ~19,975 documents and yue_ge 20,000 ids. That is defect `d1cdf9c`, which had already forced one Exp. 4 re-run, surviving in the arm that is now the same figure's other panel; both panels would have carried different units under one caption. Now one bundle per record, pinned by three tests including a cross-check that Exp. 4 and Exp. 9 agree on what a returned result is. **Manuscript:** `9266a08` was an Overleaf paste-back that silently reverted three recorded fixes — §V's repetition count 10 → 30 (REPS-1, and `test_manuscript_section_v_matches_the_config` would have failed), the trapdoor cost `O(q)T_H` → `O(q)T_P` with the sentence explaining the pairing-free construction deleted, and the verification cost back to the phantom `O(d)T_Ver` (FIX-1) while `tab:cost`'s own cell said `O(r)T_MT + O(r)T_H`. All three restored to the table cell, which is authoritative; `T_Ver` now appears only in the notation table and Scheme [54]'s row, where it is real. "Moderate per-result cost" is now "the highest of the four compared schemes", which is what the data says. Scheme [54]'s absence from panel (b) is stated with its reason rather than left for a reviewer to notice. **[results-affecting: Exp. 9 needs rerunning at the corrected unit]** |
| 2026-09-06 | **Manuscript re-synced; the code re-anchored to it; and its nine results-affecting divergences implemented as a parallel track rather than in place.** `Overleaf/MA-LB-PQ-VDSE.tex` was replaced with the current revision (eight phases, policy-state-aware index, DIAS). **§V's "repeated 30 times" was NOT carried over** — `global.yaml` is 10 and every banked run is n=10, so the draft would have claimed a replication count the data does not have; `test_repetition_count_agreement.py` exists because that exact drift happened on 2026-09-03. *Re-anchoring:* the manuscript merged Phase VII's seven steps into five and Phase VIII's six into four, so 78 `Phase VII/VIII Step N` references were remapped in one pass (old VII.2 and VII.3 both become VII.2, so sequential edits would have collided); eight module headers pointed at steps that had moved (`verify/ledger.py` VIII.3→VIII.2, `chain/ipfs.py` V.1→V.4, `user/delivery.py` III.3→III.2, `user/profile.py` III.4→III.3, `authority/keygen.py` III.2→III.1, plus `chain/outsourcing.py`, `shard/propagation.py` and `user/token.py` whose steps were merged or deleted). 22 docstrings citing the two-generations-dead `Overleaf/PQ-AVDSE-OJCOMS:NNN` and 39 further bare `:NNN` anchors were stripped — the phase and step name sits beside each, so nothing was lost but the rot. `IAS` → `DIAS` throughout (282 substitutions, `sync/ias.py` → `sync/dias.py`); Exp. 6's arms are now `VARIANT_DIAS`/`VARIANT_INCREMENTAL_ALL`/`VARIANT_FULL_STATE` and the figure legend reads DIAS / Incremental-All / Full-State Synchronization, **but their string values still read `ias`/`broadcast`/`full_rebuild`** because the value is the directory every banked run was written into — renaming it would orphan measured data to change something no reader sees. The four dated `README`/`PHASE_*` history entries that a first pass had rewritten were reverted: their `:NNN` anchors point into the superseded revision, so swapping only the filename would make each citation actively wrong. *Divergences:* `MANUSCRIPT_DIVERGENCE.md` records 14 — D1–D9 results-affecting, D10–D14 defects on the manuscript's side (now §14 item 11; note that (a) is a **regression** — the 2026-09-05 entry above records fixing that exact wording, and the new revision restored it). *The track:* `src/psa/` implements D1–D5 (`T = H(w‖PID‖PV‖Dom)`, the `V_P` vector and `PV` digest replacing the scalar `VID`, `AuthState`, `I = (T,CID,PID,PV)`, the paired VAP) and `harness/psa_experiments.py` implements D6–D9, reached by `--construction psa` and writing to `psa_exp*/`. Modelling `AA(PID_i)` was unavoidable: read literally it is a singleton and both halves of the manuscript's central claim are vacuous, so `psa/governance.py` makes the set size a `benchmark` parameter defaulting to 2-of-4 (§14 item 14). *Findings, local and provisional:* `\|T_Q\| = q·\|P_U\|` holds exactly, and cost tracks the **product alone** — three factorizations of `\|T_Q\|`=20 and of 40 agree within **1.004×–1.039×**, which supports the current `tab:cost` row `O(\|T_Q\|)T_H` and is a claim the previous table could not express, since Option D's trapdoor has no `\|P_U\|`. Exp. 3's `trapdoors_issued: 1.0` does not survive: at `d`=10 a cross-domain query issues **50** tokens against Option D's **1**, which is presumably why the manuscript dropped the claim. Those numbers came from a laptop and the banked data from AWS, so they were **deleted rather than committed** — the four `PSA_CLAIMS` cost tests skip until a campaign host produces both arms, and that skip is the correct state. Test suite **713 → 765 passed**, 9 skipped. **Not results-affecting: no `results.csv`, `raw_runs.csv`, `run_meta.json` or config file was touched.** |
