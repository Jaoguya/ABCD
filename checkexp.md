...
Suite: **894 pass, 5 skip**, and one intermittent failure — the flaky
wall-clock comparison in `needfix.md` X-4, which has now fired twice in seven
full runs. The five `README.md` failures are gone: that file was deleted on
2026-09-07 and the six tests that read it were removed with it.

| Exp | Figure | Checked | Verdict |
|-----|--------|---------|---------|
| 1 | `fig_exp1_trapdoor.pdf` | **2026-09-07** (partial) | **5 items**, 1 fixed. Code to spec, **data is not**: all four plotted arms are `reportable: false` / `psa_in_process`. A smoke run closes it |
...
| 3 | `fig_exp3_crossdomain.pdf` | **2026-09-07** (deep) | **8 items**, none fixed, 2 decisions. **Worst workload mismatch** — 2,500x index gap against the baselines |
| 4 | `fig_exp4_verify.pdf` | **2026-09-07** (partial) | **2 items**, both open. Cleanest of what was examined — axis, boundary and roster agree. Its psa arm, workload and tests are unexamined |
| 5 | `fig_exp5_update.pdf` | **2026-09-07** (partial) | **4 items**, 2 decisions. Axis sound, but `entries_rewritten` reads **0 at all four points**, and the index grows with k where §VI fixes it |
| 6 | — (no figure) | **2026-09-07** (partial) | **3 items** (2 open, 1 note). Ablation, no cross-scheme axis. Code declares 3 secondaries, banked data has 2 |
| 7 | `fig_exp7_throughput.pdf` | **2026-09-07** (partial) | **2 items**, both fixed. Data confirmed post-q-fix (`9519a02`), all six points, all four arms. But its §VI throughput claims were never checked — G-1 |
| 8 | `fig_exp8_balance.pdf` | **2026-09-07** (partial) | **1 item**, blocked. Panel (c) draws **max_queue_depth** under a "Cross-node forwards" label. Needs the re-run |
...
Totals across all sections: **54 items — 17 fixed, 22 open, 8 decisions,
5 blocked**, plus 2 standing notes.

### Coverage — how deep each pass actually went

**Only Exp. 2 and Exp. 3 are finished.** Every experiment was passed over and
every one produced findings, but the checklist was not run end-to-end on the
other six. "No further findings" is NOT a claim this table supports for
anything except 2 and 3.
...
* **Item 3** never run on Exps. 4, 6, 7, 8 → `needfix.md` **G-1**
* **Item 4** only run on Exps. 1, 2, 3; 19 banked `psa_*` dirs unchecked →
  **G-2**, and the `psa_exp5_retokenization` naming problem → **G-4**
* **Item 7** only run on Exp. 2 → **G-3**
* **Item 6** only diffed for Exp. 2; Exps. 1, 3, 5 unknown → **X-5**
