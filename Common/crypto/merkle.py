...
class MerkleTree:
    ...
    def update_leaf(self, index: int, data: bytes) -> int:
        ...
        measurement, which is what SystemConfiguration.md requires.

        Rebuilding the whole tree instead would be the Phase VII bug that
        global.yaml's Exp. 5 rule explicitly calls out.
        """
        self._check_index(index)
...
# exactly what made the proposed scheme's Exp. 6 unable to complete (SystemConfiguration.md
