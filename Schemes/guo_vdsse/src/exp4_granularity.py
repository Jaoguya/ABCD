"""Exp. 4, arm `granularity` — verification granularity under tampering.

Scheme 35's arm of Section VI Fig. 4(b). There is no Experiment 9 in the
manuscript: Exp. 4 makes a two-part claim over two variables, and this is
the second part.

Variable:  tampered records ``t`` = 1 -> 1000, at a PINNED result-set size
           (``global.yaml``: ``exp4_verification_overhead.held_constant.
           returned_results``)
...
def run(
    ...
    *,
    ...
    exp_dir = sweep.shard_dir(
        output_dir / "exp4_verification_overhead__granularity", points
    )
