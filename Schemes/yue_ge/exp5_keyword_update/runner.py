"""Exp. 5 — Dynamic Keyword Update. Scheme 30 §VI-A ``Add`` / ``Delete``.

Variable:  (keyword, document) pairs ``k`` = 10^2 -> 10^5 (skill.md)
Primary:   update latency (ms)
Secondary: entries rewritten, index growth (bytes)

INCREMENTAL, NOT A REBUILD
--------------------------
skill.md is explicit: "incremental update only. A global rebuild means Phase
VII is implemented wrong." Scheme 30 satisfies this natively — ``Add`` writes a
...
against a 2,102-word vocabulary (skill.md), so 10^5 pairs is available while
