...
same native-mode rule SystemConfiguration.md applies to Exp. 3, and the treatment
...
# global.yaml: keywords per query q = 1 -> 20
VARIABLE_RANGE = list(range(1, 21))

# Index size held at the global.yaml default. Exp. 1 sweeps q and holds every
# other parameter at its default, and global.yaml declares
...
# global.yaml default: queries are issued by a mid-level user. Level |L| would see
...
def run(
    ...
    *,
    ...
    #
    # Scoped to DEFAULT_N (global.yaml default index size) rather than the whole
