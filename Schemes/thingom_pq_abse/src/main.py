...
A run is REPORTABLE only when every one of these holds:
  ...
  * this process is actually running on the pinned AWS experiment host
    ...
    laptop or any other machine (skill.md).

Anything else still runs — development on Windows is expected (skill.md) —
...
# skill.md defaults. These live here rather than in a config file because
# "Experiment Configuration/global.yaml" — referenced by 41.md and
# skill.md — does not exist in the repository yet. Every value that affects
...
# budget on the pinned AWS host (skill.md). Ref[41] has no index structure
...
# experiments.py's _parallel_search and 41.md's Feasibility section for
...
def resolve_experiments(spec: str) -> List[str]:
    ...
    if unknown:
        raise SystemExit(
            f"Ref[41] runs experiments 1, 2 and 3 only (41.md); "
            f"got {', '.join(unknown)}"
        )
    return selected
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parameters: Dict[str, Any] = {
        ...
        # reported number MEANS. 41.md prose is not machine-readable
        # provenance (skill.md), and neither fact is otherwise recoverable
